...
RUN apt-get -y update && apt-get -y upgrade && \
    ...
    mkdir /app && \
    R -e "install.packages(c('plumber', 'DBI', 'RSQLite', 'jsonlite', 'jose', 'nnet'), Ncpus = 16, repos='https://cloud.r-project.org/')" ; \
    R -e "if(!require(plumber)) install.packages('plumber'); if(!require(DBI)) install.packages('DBI'); if(!require(jsonlite)) install.packages('jsonlite'); if(!require(jose)) install.packages('jose'); if(!require(nnet)) install.packages('nnet')" && \
